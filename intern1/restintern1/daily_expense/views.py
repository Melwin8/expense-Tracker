...
class CurrentMonthExpenseView(APIView):
    ...
    def get(self, request):
        ...
        end_date_month = current_month + 1 if current_month < 12 else 1
        end_date_year = current_year + 1 if current_month == 12 else current_year
        expense = Expense.objects.filter(
            user=user,
            date_of_transaction__gte=datetime(current_year, current_month, 1),
            date_of_transaction__lt=datetime(end_date_year, end_date_month, 1)
        )
        total_expense=0
        for i in expense:
...
class TotalAmountByCategoryInPercentageView(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request):
        user = self.request.user
        current_month = datetime.now().month
        ...
        return Response(dict(percentages))
...
class MonthExpenseReportsView(APIView):
    ...
    def get(self,request):
        ...
        end_date_month = int(selected_month) + 1 if int(selected_month)< 12 else 1
        end_date_year = int(selected_year) + 1 if int(selected_month) == 12 else int(selected_year)
...
class TotalAmountByCategoryinmonth(APIView):
    permission_classes = [IsAuthenticated]
    def get(self, request):
        user = self.request.user
        current_month = datetime.now().month
